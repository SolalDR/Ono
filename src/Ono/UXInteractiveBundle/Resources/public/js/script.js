...
textures = {
  earthnight : textureLoader.load('../asset/img/earthnight.jpg'),
  earthspec : textureLoader.load('../asset/img/earthspec1k.jpg'),
  earthlight : textureLoader.load('../asset/img/lightWhite.png'),
  starfield : textureLoader.load('../asset/img/starfield.png')
}
...
textureFlare = textureLoader.load( "../asset/img/lensflare2.jpg" );
...
function loadFont() {
  ...
  loader.load( '../asset/font/Roboto/Roboto_Regular.json', function ( response ) {
    font = response;
    createTexts();
  });
}
...
var closePopin = function(){
  if(OnoHistoryPopin.isDisplay()){
    switchOff(30);
    // clearInterval(alternateLight);
    OnoHistoryPopin.hide();
    soundVoice.stop();
    analyser = null;
    soundVoice = null;
  }
...
for(i=0; i<coord.length; i++){
  ...
  (function(){
    ...
    if(coord[r].content.voice){
      audioLoader.load( '../asset/audio/'+coord[r].content.voice.fileName, function( buffer ) {
...
window.onload = function(){
  setTimeout(function(){
    OnoHistoryPopin.init();
    genLight();
    render();
    SoundVolume.init();
    OnoHistoryLoader.loadCallBack();
  }, 2000)
}
